...
client = MecaPyClient(auth=Auth.Token("YOUR_API_KEY"))
pkg = client.load("mon-premier-package")

result = pkg.addition(x=10, y=5)
print(result)  # {'result': 15.0}
```

```bash cURL
curl -X POST \
  "https://api.mecapy.com/packages/{package_id}/functions/addition/execute" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"payload": {"x": 10, "y": 5}}'
```
...
<Tip>
  Le SDK gère automatiquement le polling du résultat. Consultez la [documentation du client Python](/sdk/python-client) pour les options avancées (jobs non-bloquants, timeout, etc.).
</Tip>
