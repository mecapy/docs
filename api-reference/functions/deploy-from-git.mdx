...
description: "Deploy a function directly from a Git repository"
---

Deploy a function from a Git repository (GitHub, GitLab, Bitbucket). This endpoint clones the repository, validates the code for security, and registers it for asynchronous execution via the Docker worker.
...
<ParamField body="handler" type="string" required>
  ...
  **Example:** `main.calculate` → calls `calculate()` in `main.py`
</ParamField>
